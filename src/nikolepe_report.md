...
## Part 1. Установка ОС
- Скачал образ Ubuntu 20.04 Server LTS с официального сайта.
- Создал новую виртуальную машину в VirtualBox без графического интерфейса.
- Установил Ubuntu 20.04 Server LTS на созданную виртуальную машину.
- После установки, выполнил команду `cat /etc/issue` для проверки версии Ubuntu.
...
#### JOE
- Для установки JOE необходимо обновить систему `sudo apt update`
...
Утилита top обычно предустановлена во многих дистрибутивах Linux. используя команду `sudo apt-get update` я обновил пакет:
...
![Вывод на экран команды top](screen/Part_9_top_result.png)
    - uptime: 5 min (время, прошедшее с момента последней загрузки системы)
    - количество авторизованных пользователей: 1 user 
    - общая загрузка системы: 0.01, 0.07, 0.04 (load average – среднее количество процесоов, ожидающих доступа к ЦП в течение последних 1, 7 и 4 минут)
    - общее количество процессов: 98 (Tasks total)
    - загрузка cpu:
        + 0.3 us: 0.3% времени процессора занято пользовательскими процессами
        + 0.3 sy: 0.3% времени процессора занято системными процессами
        + 0.0 ni: 0.0% времени процессора занято процессами с приоритетом ниже нормального
        + 99.3 id: 99.3% времени процессора простаивает, т.е. не используется
    ...
    - загрузка памяти (MiB Mem): общий объем оперативной памяти составляет 1971.6 мегабайт, из которых:
        + 1236.5 мегабайт свободны
        + 146.6 мегабайт используются
        + 588.5 мегабайт используются для буферизации и кэширования
     - pid процесса занимающего больше всего памяти: 655 
...
### Вывод команды `htop`
 Для сортировки после ввода команды `htop` нажимаем `F6` и переключаемся по элементам для выбора, затем `ENTER`)
    * отсортированный по `PID`:
    ![Вывод команды htop, отсортированный по PID](screen/Part_9_htop_PID_sorted.png)
...
![Вывод команды htop, отфильтрованный для sshd](screen/Part_9_htop_sshdfiltrated.png)
...
![ Вывод команды 'df'](screen/Part_11_df_output.png)
    ...
    + Размер занятого пространства: 4883328 килобайт
    + Размер свободного пространства: 6256324 килобайт
    + Использовано 44%
- Вывод команды `df -Th`
...
- Для открытия файлов для просмотра используем команду `tail -f`
- при выводе команды `/var/log/auth.log` видно 
* время последней успешной авторизации '13:17:07'
* имя пользователя user g
...
![Часть вывода файла /var/log/syslog с сообщениями о перезапуске службы SSHd](screen/Part_14_ssh_restart.png)
